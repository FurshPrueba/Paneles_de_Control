...
//CLICK AL ICONO DE SUMA
    ...
    $(document).on('click', '.iconoSuma', function(){
        ...
        nuevoProcedimientoTemporal.classList.add('procedimientoHijo');
        ...
        setTimeout(function() {
            ...
            $(nuevoProcedimientoTemporal).css('top', ((procedimientoActual.find('.subproceso').length + 1) * 60) + 'px');
            ...
            $(nuevoProcedimientoTemporal).addClass('subproceso');
            
            //IMPROTANTE ({[El id de el nuevo proceso se establece aca]})
            nuevoProcedimientoTemporal.id = `subproceso_${procedimientoActual.find('.numerador:first').text() + '.' +  procedimientoActual.find('.procedimientoHijo').length}`;
        ...
        //Si se da click en una area despues de darle al icono de agregar, entonces que aparesca una ventana 'emergente' en el lugar del area
            ...
            //ir a ventModaliconoMas para seguir los procedimientos
